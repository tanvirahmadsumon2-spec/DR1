    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
            <?php foreach (array_slice($specialties, 0, 12) as $spec): ?>
                <a
                    href="/doctors?specialty=<?= e($spec['slug']) ?>"
                    class="p-4 rounded-xl bg-white border border-slate-200 hover:border-emerald-500 hover:shadow-md transition group cursor-pointer text-center block"
                >
                    <div class="w-10 h-10 mx-auto rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center group-hover:scale-110 group-hover:bg-emerald-600 group-hover:text-white transition">
                        <i data-lucide="stethoscope" class="w-5 h-5"></i>
                    </div>
                    <h3 class="font-semibold text-slate-800 text-xs mt-3 truncate group-hover:text-emerald-600 transition">
                        <?= e($spec['name']) ?>
                    </h3>
                    <?php if (!empty($spec['name_bn'])): ?>
                        <p class="text-[11px] text-slate-400 mt-0.5 truncate"><?= e($spec['name_bn']) ?></p>
                    <?php endif; ?>
                </a>
            <?php endforeach; ?>
        </div>
