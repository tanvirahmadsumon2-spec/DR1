        ...
        <div class="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="flex items-center border-l border-slate-700 pl-3">
                    <a href="/lang/<?= \App\Core\Session::get('lang', 'bn') === 'bn' ? 'en' : 'bn' ?>" class="hover:text-emerald-400 transition font-medium cursor-pointer text-[11px]">
    ...
    <header class="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 no-print">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <!-- Logo -->
                <a href="/" class="flex items-center gap-2.5 group text-decoration-none">
                ...
                <nav class="hidden md:flex items-center gap-1">
                    <a href="/" class="px-3.5 py-2 rounded-lg text-sm font-medium transition <?= ($_SERVER['REQUEST_URI'] ?? '/') === '/' ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' ?>">
                        <?= t('Home', 'হোম') ?>
                    </a>
                    <a href="/doctors" class="px-3.5 py-2 rounded-lg text-sm font-medium transition <?= str_starts_with($_SERVER['REQUEST_URI'] ?? '', '/doctors') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' ?>">
                        <?= t('Find Doctors', 'ডাক্তার খুঁজুন') ?>
                    </a>
                    <a href="/#how-it-works" class="px-3.5 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition">
                    ...
                    <?php if (authCheck()): ?>
                        ...
                        <?php if ($u['role'] === 'patient'): ?>
                            <a href="/patient/dashboard" class="px-3.5 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1.5 <?= str_starts_with($_SERVER['REQUEST_URI'] ?? '', '/patient') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' ?>">
                        ...
                        <?php elseif ($u['role'] === 'doctor'): ?>
                            <a href="/doctor/dashboard" class="px-3.5 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1.5 <?= str_starts_with($_SERVER['REQUEST_URI'] ?? '', '/doctor') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' ?>">
                                <i data-lucide="stethoscope" class="w-4 h-4 text-emerald-600"></i>
                                <span><?= t('Doctor Portal', 'ডাক্তার ড্যাশবোর্ড') ?></span>
                            </a>
                        <?php elseif ($u['role'] === 'admin'): ?>
                            <a href="/admin/dashboard" class="px-3.5 py-2 rounded-lg text-sm font-medium transition flex items-center gap-1.5 <?= str_starts_with($_SERVER['REQUEST_URI'] ?? '', '/admin') ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' ?>">
                ...
                <div class="flex items-center gap-3">
                    <?php if (authCheck()): ?>
                        ...
                        <div class="relative group">
                            ...
                            <div class="hidden group-hover:block absolute right-0 mt-1 w-56 bg-white rounded-xl shadow-xl border border-slate-100 py-1 z-50">
                                ...
                                <?php if ($u['role'] === 'patient'): ?>
                                    <a href="/patient/dashboard" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-slate-50">
                                        <i data-lucide="calendar-check" class="w-4 h-4 text-slate-400"></i>
                                        <span><?= t('My Appointments', 'আমার অ্যাপয়েন্টমেন্ট') ?></span>
                                    </a>
                                    <a href="/patient/profile" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-slate-50">
                                        <i data-lucide="user" class="w-4 h-4 text-slate-400"></i>
                                        <span><?= t('My Profile', 'আমার প্রোফাইল') ?></span>
                                    </a>
                                <?php elseif ($u['role'] === 'doctor'): ?>
                                    <a href="/doctor/dashboard" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-slate-50">
                                        <i data-lucide="layout-dashboard" class="w-4 h-4 text-slate-400"></i>
                                        <span><?= t('Doctor Portal', 'ডাক্তার ড্যাশবোর্ড') ?></span>
                                    </a>
                                    <a href="/doctor/patients" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-slate-50">
                                        <i data-lucide="users" class="w-4 h-4 text-slate-400"></i>
                                        <span>Patient History</span>
                                    </a>
                                    <a href="/doctor/profile" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-slate-50">
                                        <i data-lucide="user" class="w-4 h-4 text-slate-400"></i>
                                        <span>Edit Profile</span>
                                    </a>
                                <?php elseif ($u['role'] === 'admin'): ?>
                                    <a href="/admin/dashboard" class="flex items-center gap-2 px-4 py-2 text-xs text-slate-700 hover:bg-slate-50">
                                        <i data-lucide="shield-check" class="w-4 h-4 text-slate-400"></i>
                                        <span><?= t('Admin Panel', 'অ্যাডমিন প্যানেল') ?></span>
                                    </a>
                                <?php endif; ?>
                                <div class="border-t border-slate-100 my-1"></div>
                                <a href="/logout" class="flex items-center gap-2 px-4 py-2 text-xs text-rose-600 hover:bg-rose-50">
                    ...
                    <?php else: ?>
                        <div class="flex items-center gap-2">
                            <a href="/login" class="px-3.5 py-1.5 text-sm font-medium text-slate-700 hover:text-emerald-700 hover:bg-slate-100 rounded-lg transition">
                                <?= t('Sign In', 'সাইন ইন') ?>
                            </a>
                            <a href="/register/patient" class="px-3.5 py-1.5 text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg shadow-sm transition">
                                <?= t('Register', 'নিবন্ধন') ?>
                            </a>
                            <a href="/register/doctor" class="hidden sm:inline-flex text-xs text-slate-500 hover:text-emerald-700 font-medium ml-1">
                                For Doctors &rarr;
                            </a>
                        </div>
                    <?php endif; ?>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        <li><a href="/doctors" class="hover:text-white transition"><?= t('Search Specialist Doctors', 'বিশেষজ্ঞ ডাক্তার খুঁজুন') ?></a></li>
                        <li><a href="/doctors" class="hover:text-white transition"><?= t('Book Chamber Serial', 'চেম্বার সিরিয়াল বুক করুন') ?></a></li>
                        <li><a href="/patient/dashboard" class="hover:text-white transition"><?= t('Check Serial Status', 'সিরিয়াল স্ট্যাটাস চেক করুন') ?></a></li>
                        <li><a href="/patient/dashboard" class="hover:text-white transition"><?= t('Download Appointment Slip', 'অ্যাপয়েন্টমেন্ট স্লিপ ডাউনলোড') ?></a></li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-2">
                        <li><a href="/register/doctor" class="hover:text-white transition"><?= t('Doctor Registration', 'ডাক্তার রেজিস্ট্রেশন') ?></a></li>
                        <li><a href="/doctor/dashboard" class="hover:text-white transition"><?= t('Manage Chambers & Schedules', 'চেম্বার ও সময়সূচী নির্ধারণ') ?></a></li>
                        <li><a href="/doctor/dashboard" class="hover:text-white transition"><?= t('Live Serial & Queue Management', 'লাইভ সিরিয়াল ব্যবস্থাপনা') ?></a></li>
                        <li><a href="/#how-it-works" class="hover:text-white transition"><?= t('Double Booking Safeguards', 'ডাবল বুকিং সুরক্ষা') ?></a></li>
                    </ul>
                </div>
